...
const Assign_case = ({ onChange }) => {
  ...
  const handleClientChange = (e) => {
    ...
    setSelectedClient(clientName); // Optional if you still need the state for UI logic
  
    const client = caseData.find((c) => c.customer_name === clientName);
  
    if (client) {
      setSelectedCaseId(client.case_id); // Optional: only if you're still showing it somewhere else
      setValue("case_id", client.case_id, { shouldValidate: true, shouldDirty: true }); // ✅ This line is key
    } else {
      setSelectedCaseId(""); // Optional
      setValue("case_id", "", { shouldValidate: true, shouldDirty: true });
    }
  };
  ...
  }, [cases]); // Run this effect only when `cases` changes
                              ...
                              <div className="col-md-6" hidden>
                                <div className="form-group">
                                  ...
                                  <input
                                    ...
                                    name="case_id"
                                    value={selectedCaseId} // Controlled by state
                                    onChange={(e) => {
                                      setSelectedCaseId(e.target.value); // Update state
                                      setValue("case_id", e.target.value, { shouldValidate: true, shouldDirty: true }); // Update form state
                                    }}
                                    {...register("case_id", { required: "Case ID is required" })} // Register the field
                                  />
                                  ...
                                  >
                                    <li>
                                      <Link
                                        ...
                                        onClick={() =>
                                          handleViewProfile(casses.passport_no)
                                        }
                                      >
                                        Track
                                      </Link>
